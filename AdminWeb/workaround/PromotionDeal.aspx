<%@ Page Language="C#" AutoEventWireup="true" CodeFile="PromotionDeal.aspx.cs" Inherits="workaround_PromotionDeal" %>
             ...
             <script language="javascript" >
               function OpenPrint(id)
                ...
                function OpenDetail(id)
                {
	                var features = 'width=800,height=700,scrollbars=yes,left=50,top=50,resizable=yes';
	                var winName = '报批申请表明细';
	                var Url = 'PromotionDealDetail.aspx?id=' + id;
	                var win = window.open(Url, winName, features);
	                win.focus();
                }
</script>
...
<body>
  ...
  <tr> 
    ...
    <table width="96%" border="0" align="center" cellpadding="0" cellspacing="0">
        ...
        <tr> 
          <td class="left-bt" colspan="3">&nbsp;<img src="../img/body_ico_bt.gif" width="6" height="6">&nbsp;<asp:Label runat="server" ID="lblSortName">处理通知书列表</asp:Label></td>
        </tr>
		<tr> 
        ...
        <tr> 
         ...
         <asp:GridView ID="gvList" runat="server" Width="100%" BorderWidth="0" cellpadding="0" cellspacing="1"
	            ...
	            <Columns>		           
		            ...
		            <asp:TemplateField HeaderText="企业名称" ItemStyle-Width="44%">
		            ...
		            <asp:TemplateField HeaderText="编辑" ItemStyle-Width="4%">
		                <ItemStyle HorizontalAlign="Center" />
			            <ItemTemplate>
			            <a class="link-02" style="cursor:hand;display:<%#((System.Data.DataRowView)Container.DataItem)["PromotionDealID"].ToString()==""?"none":"table-row" %>;"  href="PromotionDeal_edit.aspx?id=<%#((System.Data.DataRowView)Container.DataItem)["ID"]%>" >编辑</a>		           
			            </ItemTemplate>
		            </asp:TemplateField>
		            <asp:TemplateField HeaderText="打印" ItemStyle-Width="4%">
		                <ItemStyle HorizontalAlign="Center" />
			            <ItemTemplate>
			            <a class="link-02" style="cursor:hand;display:<%#((System.Data.DataRowView)Container.DataItem)["PromotionDealID"].ToString()==""?"none":"table-row" %>;"  href="javascript:OpenPrint('<%#((System.Data.DataRowView)Container.DataItem)["ID"]%>')" >打印</a>			           	           
			            </ItemTemplate>
		            </asp:TemplateField>
		            <asp:TemplateField HeaderText="完成" ItemStyle-Width="8%">
		                <ItemStyle HorizontalAlign="Center" />
			            <ItemTemplate>
				        <asp:LinkButton runat="server" CssClass="link-02"  CommandArgument='<%# ((System.Data.DataRowView)Container.DataItem)["ID"]%>' ID="LinkButton1" CommandName="pass" Text="完成" OnLoad="LinkCategory_Load1" Visible='<%#((System.Data.DataRowView)Container.DataItem)["PromotionDealID"].ToString()!="" %>' ></asp:LinkButton>				
				         <asp:LinkButton runat="server" CssClass="link-02"  CommandArgument='<%# ((System.Data.DataRowView)Container.DataItem)["ID"]%>' ID="LinkButton2" CommandName="set" Text="生成通知书" Visible='<%#((System.Data.DataRowView)Container.DataItem)["PromotionDealID"].ToString()=="" %>' ></asp:LinkButton>
			            </ItemTemplate>
		            </asp:TemplateField>
		            <asp:TemplateField HeaderText="删除" ItemStyle-Width="5%">
		                <ItemStyle HorizontalAlign="Center" />
			            <ItemTemplate>			        
				        <asp:LinkButton runat="server" CssClass="link-02"  CommandArgument='<%# ((System.Data.DataRowView)Container.DataItem)["PromotionDealID"]%>' ID="LinkButton3" CommandName="del" Text="删除" OnLoad="LinkCategory_Load2" Visible='<%#((System.Data.DataRowView)Container.DataItem)["PromotionDealID"].ToString()!="" %>' ></asp:LinkButton>
		                </ItemTemplate>
		            </asp:TemplateField>
	            </Columns>
            </asp:GridView>
