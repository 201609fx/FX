<%@ Page Language="C#" AutoEventWireup="true" CodeFile="result.aspx.cs" Inherits="workaround_result" %>
 ...
 <script language="javascript" >
               function OpenCheck(id)
                ...
                function OpenDetail(id)
                {
	                var features = 'width=800,height=700,scrollbars=yes,left=50,top=50,resizable=yes';
	                var winName = '报批申请表明细';
	                var Url = 'resultDetail.aspx?id=' + id;
	                var win = window.open(Url, winName, features);
	                win.focus();
                }
</script>
        ...
        <tr> 
         ...
         <asp:GridView ID="gvList" runat="server" Width="100%" BorderWidth="0" cellpadding="0" cellspacing="1"
	            ...
	            <Columns>		           
		            ...
		            <asp:TemplateField HeaderText="企业名称" ItemStyle-Width="45%">
		            ...
		            <asp:TemplateField HeaderText="申请书编号" ItemStyle-Width="10%">
		               ...
		               <asp:TemplateField HeaderText="评审日期" ItemStyle-Width="10%">
		                <ItemStyle HorizontalAlign="left"/>
			            <ItemTemplate>
				            &nbsp;<%#getTime(((System.Data.DataRowView)Container.DataItem)["PSTime"].ToString())%>
			            </ItemTemplate>
		            </asp:TemplateField>
		            <asp:TemplateField HeaderText="管理" ItemStyle-Width="11%">
		                <ItemStyle HorizontalAlign="Center" />
			            <ItemTemplate>	
				        <asp:LinkButton runat="server" CssClass="link-02"  CommandArgument='<%# ((System.Data.DataRowView)Container.DataItem)["ID"].ToString()+","+((System.Data.DataRowView)Container.DataItem)["Type"].ToString()%>' ID="LinkButton1" CommandName="pass" Text="完成" OnLoad="LinkCategory_Load1" Visible='<%#((System.Data.DataRowView)Container.DataItem)["ResultID"].ToString()!="" %>' ></asp:LinkButton>
				         <asp:LinkButton runat="server" CssClass="link-02"  CommandArgument='<%# ((System.Data.DataRowView)Container.DataItem)["ID"]%>' ID="LinkButton2" CommandName="set" Text="生成结论表" Visible='<%#((System.Data.DataRowView)Container.DataItem)["ResultID"].ToString()=="" %>' ></asp:LinkButton>
			            <a class="link-02" style="cursor:hand;display:<%#((System.Data.DataRowView)Container.DataItem)["ResultID"].ToString()==""?"none":"table-row" %>;"  href="javascript:OpenCheck('<%#((System.Data.DataRowView)Container.DataItem)["ID"]%>')" >打印</a>			          
			            </ItemTemplate>
		            </asp:TemplateField>
		            <asp:TemplateField HeaderText="编辑" ItemStyle-Width="5%">
		                <ItemStyle HorizontalAlign="Center" />
			            <ItemTemplate>
			            <a class="link-02" style="cursor:hand;display:<%#((System.Data.DataRowView)Container.DataItem)["ResultID"].ToString()==""?"none":"table-row" %>;"  href="Result_Edit.aspx?id=<%#((System.Data.DataRowView)Container.DataItem)["ID"]%>" >编辑</a>				          
			            </ItemTemplate>
		            </asp:TemplateField>
		            <asp:TemplateField HeaderText="删除" ItemStyle-Width="5%">
		                <ItemStyle HorizontalAlign="Center" />
			            <ItemTemplate>
			            <asp:LinkButton runat="server" CssClass="link-02"  CommandArgument='<%# ((System.Data.DataRowView)Container.DataItem)["ID"]%>' ID="LinkButton3" CommandName="del" Text="删除" OnLoad="LinkCategory_Load2"  Visible='<%#((System.Data.DataRowView)Container.DataItem)["ResultID"].ToString()!="" %>' ></asp:LinkButton>
			            </ItemTemplate>
		            </asp:TemplateField>
	            </Columns>
            </asp:GridView>
